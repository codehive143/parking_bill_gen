...
def billed():
    ...
    <html>
    <head>
        <title>Billed Records</title>
        <style>
            body {{ font-family: Arial, sans-serif; margin: 20px; }}
            .record {{ border: 1px solid #ccc; padding: 15px; margin: 10px 0; border-radius: 5px; }}
    ...
    <body>
        ...
        {"".join([f'<div class="record"><strong>{r["name"]}</strong> - {r["vehicle_no"]} - {r["month"]} {r["year"]} - {r["slot_number"]}</div>' for r in records])}
    </body>
    </html>
    """
    return simple_html
...
def handler(request, context):
    # This allows Flask to handle Vercel serverless requests
